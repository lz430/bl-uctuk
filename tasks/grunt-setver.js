...
module.exports = function(grunt) {
  grunt.registerMultiTask('setver', 'Update all version information.', function() {
		var childProcess = require('child_process'), fs = require('fs'), path = require('path'), async = require('async'), done = this.async(), currentVersion = grunt.file.readJSON('package.json').version, self = this, applyVersion = function(newver) {
			if (!newver)
          ...
          thumbnail: function(cb) {
            ...
            child.stderr.on('data', function(data) {
              grunt.log.error("Adding version to thumbnail image failed. Please ensure ImageMagick is installed.");
              cb(data);
            });
