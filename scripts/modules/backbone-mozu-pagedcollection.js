define(["jquery", "underscore", "hyprlive", "modules/backbone-mozu-model"], function($, _, Hypr, Backbone) {

    var defaultPageSize = Hypr.getThemeSetting('defaultPageSize');

    var sorts = [{
            "text" : Hypr.getLabel('sortByMostPopular'),
            "value" : "tenant~popularity desc"
        }, {
    ...
    var PagedCollection = Backbone.MozuPagedCollection = Backbone.MozuModel.extend({
        ...
        buildRequest : function() {
            var conf = this.baseRequestParams ? _.clone(this.baseRequestParams) : {}, pageSize = this.get("pageSize"), startIndex = this.get("startIndex"), sortBy = $.deparam().sortBy || this.currentSort() || this.defaultSort;
        ...
        previousPage : function() {
            ...
            if (!this._isPaging) {
                this._isPaging = true;
                try {
                    result = this.apiModel.prevPage(this.lastRequest);
                } catch (e) {
                }
        ...
        nextPage : function() {
            ...
            if (!this._isPaging) {
                this._isPaging = true;
                try {
                    result = this.apiModel.nextPage(this.lastRequest);
                } catch (e) {
                }
                this._isPaging = false;
            }
        ...
        middlePageNumbers : function() {
            var current = this.currentPage(), ret = [], pageCount = this.get('pageCount'), i = Math.max(Math.min(current - 2, pageCount - 4), 2), last = Math.min(i + 5, pageCount);
            while (i < last)
            ret.push(i++);
            return ret;
        },
